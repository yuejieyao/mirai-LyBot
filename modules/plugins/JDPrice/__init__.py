        ...
        if re.match(p_add, msg_display):
            try:
                ...
                if not ds.existsFollow(goods_id=goods_id, group=group, qq=target):
                    ...
                    MiraiMessageRequest().sendGroupMessage(
                        msg=MessageChain([Plain(text=f"成功加入购物车~\n商品当前信息如下:\n商品地址:{url}\n"), Image(
                            image_type='group', file_path=path)]), target=group, quote=quote)
                else:
                    MiraiMessageRequest().sendGroupMessage(msg=MessageChain([Plain(text="您已添加该商品,请勿重复添加")]),
            ...
            try:
                ...
                if len(res) > 0:
                    ...
                    MiraiMessageRequest().sendGroupMessage(msg=MessageChain([Plain(text="您尚未添加过商品")]), target=group,
                                                           quote=quote)
            ...
            try:
                ...
                if ds.existsFollow(goods_id=goods_id, group=group, qq=target):
                    if ds.removeFollow(goods_id=goods_id, group=group, qq=target):
                        MiraiMessageRequest().sendGroupMessage(msg=MessageChain([Plain(text="删除成功")]), target=group,
                                                               quote=quote)
                else:
                    MiraiMessageRequest().sendGroupMessage(msg=MessageChain([Plain(text="您尚未添加该商品")]), target=group,
                                                           quote=quote)
            except:
                log.error(msg=traceback.format_exc())
