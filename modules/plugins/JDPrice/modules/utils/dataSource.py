...
class DataSource(Sqlite):
    ...
    def addFollow(self, goods_id: int, group: int, qq: int):
        ...
        return self.execute(
            "insert into follow (jd_goods_id,title,url_img,url_goods,group_id,qq_id) values(?,?,?,?,?,?)", [
    ...
    def removeFollow(self, goods_id: int, group: int, qq: int):
        return self.execute('delete from follow where jd_goods_id=:goods_id and group_id=:group and qq_id=:qq',
                            {'goods_id': goods_id, 'group': group, 'qq': qq})

    def existsFollow(self, goods_id: int, group: int, qq: int):
        res = self.query(
            'select * from follow where jd_goods_id=:goods_id and group_id=:group and qq_id=:qq',
            {'goods_id': goods_id, 'group': group, 'qq': qq})
        if len(res) > 0:
            return True
        else:
    ...
    def isPriceChange(self, goods_id: int) -> bool:
        last = self.query(
            "select price,price_plus,discont,coupon from price where jd_goods_id=:goods_id order by date desc limit 1",
            {'goods_id': goods_id})[0]
        _, price, price_plus, discont, coupon = self.utils.getGoodsPrice(goods_id)
    ...
    def create_current_img(self, goods_id: int):
        ...
        if price_last:
            ...
            height_last = h_title + 15 + (350 if sum([h_price_last + h_discont_last + h_coupon_last]) < 350 else sum(
                [h_price_last + h_discont_last + h_coupon_last]))
        ...
        if width < w_title:
            width = w_title
        height = h_title + 15 + (
            350 if sum([h_price + h_discont + h_coupon]) < 350 else sum([h_price + h_discont + h_coupon]))
