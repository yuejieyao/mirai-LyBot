    ...
    def process(self, chains: MessageChain, group: int, target: int, quote: int):
        ...
        if msg_display in ('/插件', '/plugin'):
            try:
                ...
                x.add_rows(plugins)
                MiraiMessageRequest().sendGroupMessage(msg=MessageChain([Plain('输入:/查看插件 注册名称 即可查看对应插件说明\n'),
                                                                         Plain(text='输入:/打开或关闭插件 注册名称即可切换插件状态\n'),
                                                                         Image(image_type="group",
                                                                               file_path=common.text_to_img(
                                                                                   x.get_string(vertical_char='l')))]),
                                                       target=group)
            except Exception as e:
                log.error(msg=traceback.format_exc())
                MiraiMessageRequest().sendGroupMessage(msg=MessageChain([Plain(text=str(e))]), target=group)
            ...
            try:
                ...
                MiraiMessageRequest().sendGroupMessage(msg=MessageChain([Plain('输入:/查看轮询 注册名称 即可查看对应轮询服务说明\n'),
                ...
                log.error(msg=traceback.format_exc())
                MiraiMessageRequest().sendGroupMessage(msg=MessageChain([Plain(text=str(e))]), target=group)
        elif re.match('/打开插件 .*', msg_display) is not None or re.match('/openP .*', msg_display) is not None:
            try:
                ds = MiraiDataSource()
                ...
                if ds.existGroupPlugin(register_name=msgs[1]):
                    ...
                    MiraiMessageRequest().sendGroupMessage(msg=MessageChain([Plain(text=f'成功打开插件{msgs[1]}')]),
                                                           target=group)
                ...
                log.error(msg=traceback.format_exc())
                MiraiMessageRequest().sendGroupMessage(msg=MessageChain([Plain(text=str(e))]), target=group)
        elif re.match('/关闭插件 .*', msg_display) is not None or re.match('/closeP .*', msg_display) is not None:
            try:
                ds = MiraiDataSource()
                msgs = msg_display.split(' ')
                # 确认插件存在
                if ds.existGroupPlugin(register_name=msgs[1]):
                    if msgs[1] != 'Control':
                        ...
                        MiraiMessageRequest().sendGroupMessage(msg=MessageChain([Plain(text=f'成功关闭插件{msgs[1]}')]),
                                                               target=group)
                        log.info(f'[Control][(Group){group}] close plugin {msgs[1]}')
                    else:
                        MiraiMessageRequest().sendGroupMessage(msg=MessageChain([Plain(text='你不能,也不应该关闭Control')]),
                                                               target=group)
                else:
                    raise Exception('不存在的插件')
            except Exception as e:
                log.error(msg=traceback.format_exc())
                MiraiMessageRequest().sendGroupMessage(msg=MessageChain([Plain(text=str(e))]), target=group)
        elif re.match('/查看插件 .*', msg_display) is not None or re.match('/showP .*', msg_display) is not None:
            try:
                ds = MiraiDataSource()
                msgs = msg_display.split(' ')
                if ds.existGroupPlugin(register_name=msgs[1]):
                ...
                log.error(msg=traceback.format_exc())
                MiraiMessageRequest().sendGroupMessage(msg=MessageChain([Plain(text=str(e))]), target=group)
        elif re.match('/打开轮询 .*', msg_display) is not None or re.match('/openS .*', msg_display) is not None:
            try:
                ds = MiraiDataSource()
                msgs = msg_display.split(' ')
                # 确认轮询服务存在
                if ds.existSchedule(register_name=msgs[1]):
                    ...
                    MiraiMessageRequest().sendGroupMessage(msg=MessageChain([Plain(text=f'成功打开轮询{msgs[1]}')]),
                                                           target=group)
                    log.info(f'[Control][(Group){group}] open plugin {msgs[1]}')
                else:
                ...
                log.error(msg=traceback.format_exc())
                MiraiMessageRequest().sendGroupMessage(msg=MessageChain([Plain(text=str(e))]), target=group)
        elif re.match('/关闭轮询 .*', msg_display) is not None or re.match('/closeS .*', msg_display) is not None:
            try:
                ds = MiraiDataSource()
                msgs = msg_display.split(' ')
                # 确认插件存在
                if ds.existSchedule(register_name=msgs[1]):
                    ...
                    MiraiMessageRequest().sendGroupMessage(msg=MessageChain([Plain(text=f'成功关闭轮询{msgs[1]}')]),
                                                           target=group)
                ...
                log.error(msg=traceback.format_exc())
                MiraiMessageRequest().sendGroupMessage(msg=MessageChain([Plain(text=str(e))]), target=group)
        elif re.match('/查看轮询 .*', msg_display) is not None or re.match('/showS .*', msg_display) is not None:
            try:
                ds = MiraiDataSource()
                msgs = msg_display.split(' ')
                if ds.existSchedule(register_name=msgs[1]):
